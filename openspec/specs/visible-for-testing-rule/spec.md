# visible-for-testing-rule Specification

## Purpose

The `UseVisibleForTestingAnnotation` rule: which package-private methods must carry
`@VisibleForTesting`, simple-name annotation matching, the overriding and test-method exemptions,
why this is a custom rule rather than configuration of PMD's `CommentDefaultAccessModifier`, and its
