...
`value`, `values`, `valueOf`, `empty`, `create`, `builder`, `parse`, `now`, `between`,
...
- **WHEN** a method body contains both `Arrays.copyOf(a, n)` and `List.copyOf(xs)`
- **THEN** the rule reports no violation on either

#### Scenario: A single owner of that name is reported
- **WHEN** a method body contains only `Arrays.copyOf(a, n)`
