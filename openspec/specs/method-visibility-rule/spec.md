...
The `AvoidPrivateAndProtectedMethods` rule: the two legal method visibilities, the constructor
and `@Override` exemptions, the cascade with `StaticMethodsModifyStaticState`, why the cross-module
subclass check it does not attempt is not implementable in PMD, and its documentation, test and
