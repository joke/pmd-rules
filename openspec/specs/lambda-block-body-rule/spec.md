# lambda-block-body-rule Specification

## Purpose

The `AvoidLambdaBlockBodies` rule: which lambdas it reports, the empty-block exemption, the
deliberate limit that it says nothing about logic inside an expression body, its minimum language
version, the documented conflict with `static-method-state-rule`, and its documentation, test and
mutation-coverage obligations.
